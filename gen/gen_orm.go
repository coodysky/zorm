...
func (g *gen) genRmByRule() {
	...
	g.B.WL("")
	g.B.WL("splitRules := db.SplitRulesToGroups(rules...)")
	g.B.WL("")

	// params

	g.B.WL(`isCondNotEmpty := false`)
	g.B.WL(`for _, valueRules := range splitRules {`)
	g.B.WL(`	if isCondNotEmpty {`)
	g.B.WL(`		query += " or "`)
	g.B.WL(`	} else {`)
	g.B.WL(`		isCondNotEmpty = true`)
	g.B.WL(`	}`)
	g.B.WL(`	query += " ( "`)
	g.B.WL(`	for i, rule := range valueRules {`)
	g.B.WL(`		if i > 0 {`)
	g.B.WL(`			query += " and "`)
	g.B.WL(`		}`)
	g.B.WL(``)
...
func (g *gen) genFindByMultiJoin() {
	...
	g.B.WL(``)
	g.B.WL(`	splitRules := db.SplitRulesToGroups(join.Rule...)`)
	g.B.WL(``)
	g.B.WL(`	isCondNotEmpty := false`)
	g.B.WL(`	for _, valueRules := range splitRules {`)
	g.B.WL(`		if isCondNotEmpty {`)
	...
	g.B.WL(`			query += rule.S`)
	g.B.WL(`			if rule.P != nil {`)
	g.B.WL(`				params = append(params, rule.P)`)
	g.B.WL(`			}`)
	g.B.WL(`		}`)
	g.B.WL(`		query += " ) "`)
	g.B.WL(`	}`)
	...
	g.B.WL(`splitRules := db.SplitRulesToGroups(where...)`)
	g.B.WL(``)
	g.B.WL(`isCondNotEmpty := false`)
	g.B.WL(`for _, valueRules := range splitRules {`)
	g.B.WL(`	if isCondNotEmpty {`)
